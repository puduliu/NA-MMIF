...
def fuse(): 
    ...
    model.load_state_dict(torch.load(f'model/CT_MRI.pth'))
    model.eval()
    model = model.cuda()
    ...
    datasets = ['CT-MRI-adaptive-pixel']

    dataset = 'CT-MRI-adaptive-pixel'
    method = 'NA-MMIF'
    ...
    for output_path, path1, path2 in path_pairs:
        ...
        for index, (img_path1, img_path2) in enumerate(zip(img_paths1, img_paths2)):
            ...
            img = Image.fromarray(img).convert('L')  # 保存为灰度图
            print("----------------------save_name = ", save_name)
            img.save(save_name)

    fuseY2RGB(path1, path2, output_path, output_path) # TODO 转Ycbcr试试
